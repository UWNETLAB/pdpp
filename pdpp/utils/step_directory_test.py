...
class ExistingStepDirectory(click.types.StringParamType):
    """
    Custom type for directories to check that it is already an existing directory, and that it is a proper step directory (meaning it includes input, output, and src subdirectories).
    """

    def convert(self, value, param, ctx):
        directory = rem_slash(super(ExistingStepDirectory, self).convert(value, param, ctx))

        if directory not in os.listdir():
            raise self.fail("This directory does not exist.\nPlease choose an existing step directory.", param, ctx) # type: ignore

        elif not isdir(join(directory, 'src')) or not isdir(join(directory, 'input')) or not isdir(join(directory, 'output')):
            raise self.fail("This is not a valid step directory.\n"
                            "Please choose an existing step directory that you created by running pdpp step.")# type: ignore

        return directory


class StepDirectory(click.types.StringParamType):
    """
    ...
    def convert(self, value, param, ctx):
        directory = super(StepDirectory, self).convert(value, param, ctx)
        if ' ' in directory:

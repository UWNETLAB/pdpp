...
def depgraph(files="png", gs=default_graph_style):
    ...
    """
    This section populates the graph with nodes and edges from dependency
    ...
    """
    This section creates the SPARSE graph, consisting only of edges indicating
    ...
    SPARSE.add_edges_from(edges, color=gs.EDGE_COLOR, penwidth=gs.EDGE_PEN_WIDTH)
    ...
    """
    The SOURCE graph can be built out from the SPARSE graph; it simply adds source
    files and draws edges between them and their tasks
    """
    ...
    """
    The FILE graph is built from scratch, using edges to represent the connections
    between tasks and the files that they have as either targets (implicitly defined as
    a file they output that another task relies upon) or dependencies (defined explicitly)
    """
    ...
    for task in all_tasks:
        ...
        for dep_dataclass in task.dep_files.values():
            ...
            for dep_file in dep_dataclass.file_list:
                dep_name = join(
                    dep_dataclass.task_name, dep_dataclass.task_out, dep_file
                )
                FILE.add_node(
                    dep_name,
            ...
            for dep_dir in dep_dataclass.dir_list:
                dep_name = join(
                    dep_dataclass.task_name, dep_dataclass.task_out, dep_dir
                )
                dep_label = dep_dir + "/"
                FILE.add_node(
                    dep_name,
